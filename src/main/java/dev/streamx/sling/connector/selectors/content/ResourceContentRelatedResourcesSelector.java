...
public class ResourceContentRelatedResourcesSelector implements RelatedResourcesSelector {
  ...
  private List<ResourceInfo> getRelatedResources(String resourcePath, ResourceResolver resourceResolver) {
    ...
    return extractedPaths.stream()
        .map(extractedPath -> toResourceInfo(extractedPath, resourceResolver))
        .collect(Collectors.toUnmodifiableList());
  }
  ...
  private static ResourceInfo toResourceInfo(String resourcePath, ResourceResolver resourceResolver) {
    String primaryNodeType = ResourceFilter.extractPrimaryNodeType(resourcePath, resourceResolver);
    return new ResourceInfo(resourcePath, primaryNodeType);
  }
}
